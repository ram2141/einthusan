...
def add_movies_to_list(movie_ids, bluray):
    ...
    COVER_BASE_URL = 'https://einthusan.ca/images/covers/'
    if (bluray):
        BASE_URL = 'https://einthusan.ca/movies/watch.php?bluray=true&id='
    else:
        BASE_URL = 'https://einthusan.ca/movies/watch.php?id='
...
def show_featured_movies(name, url, language, mode):
    ...
    for img, id, name, ishd in allmatches:
        ...
        link = 'https://einthusan.ca'+str(id)
        image = 'http:'+img
...
def list_music_videos(name, url, language, mode):
    if (url == "" or url == None):
        url = 'https://einthusan.ca/music/index.php?lang=' + language 
